...
import { RNGPGSAuth, RNGPGSPlayer, RNGPGSLeaderboard, RNGPGSAchievement } from 'react-native-gpgs';
import {
	StyleSheet,
	Text,
	Button,
...
export default class App extends Component {
	...
	constructor(props) {
		...
		this.authListener = RNGPGSAuth.onAuthStateChanged(signedIn => {
			this.setState({
				isSignedIn: signedIn,
				playerInfo: ''
			});
			...
			if (signedIn) {
				RNGPGSPlayer.getCurrentPlayerInfo().then(res => {
					this.setState({
	...
	componentDidMount() {
		...
		RNGPGSAuth.signInPlayerInBackground(false).then(res => {
			alert("Welcome back!");
		}).catch(err => {
	...
	_onLoginPressed = () => {
		...
		RNGPGSAuth.signInPlayerWithUI().then(res => {
	...
	_onAllLeaderboardsPressed = () => {
		RNGPGSLeaderboard.showAllLeaderboardsUI().then(() => {})
		.catch(() => {});
	}

	_onSingleLeaderboardPressed = () => {
		RNGPGSLeaderboard.showLeaderboardUI('CgkI1f2B8a4XEAIQAg').then(() => {})
		.catch(() => {});
	}

	_onAllAchievementsPressed = () => {
		RNGPGSAchievement.showAchievementsUI().then(() => {})
		.catch(() => {});
	}
